...
export class WebsocketAuthService {
  ...
  private async resumeSession(): Promise<void> {
    ...
    if (response.accountIds.length === 0) {
      ...
      const mainAccountId = parseProtobufLocalId(response.mainAccountId!);
      log.info(`✅ Account ${mainAccountId} is available (${response.accountIds.length} account(s) available)`);
      this.eventEmitter.sendEvent('loginStateSynced', {
        mainAccountId: mainAccountId,
        loggedInAccountIds: response.accountIds.map(id => parseProtobufLocalId(id)),
      });
